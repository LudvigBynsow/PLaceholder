...
<script>
    ...
    if (matchCount==2){
        if (bluePoints>redPoints){
            alert("Blue player has won the match with "+ bluePoints + " points! Restart to play again")
        }
        if (bluePoints==redPoints){
            alert("Tie! Restart to play again")
        }
        else{
            alert("Red player has won the match with "+ redPoints + " points! Restart to play again")
        }
    }
    }
...
<input type="button" value="starta om" onclick={window.location.reload()}>
